...
=======
Soit $E=\frac{1}{m}\sum 1_{z_i\ne z_i}$, $T_i=1_{z_i\ne z_i}$ suit une loi de Bernouilli de paramètre $\varepsilon$.
...
Un intervalle de confiance pour l'espérance de E, comme les taux d'erreur $E_j$ sont indépendants, est : $IC = [\bar{E} - t_{m-1;1-\alpha/2}\frac{S^*}{\sqrt{m}}, \bar{E} + t_{m-1;1-\alpha/2}\frac{S^*}{\sqrt{m}}]$
...
\begin{tabular}{|c|c|c|c|c|}
\hline
\multicolumn{2}{|c|}{Synth1} & $\varepsilon apprentissage$ & $\varepsilon test$ & Intervalle de confiance  \\
\hline
40 & $\alpha$ & (-1.904681,0.698841) & $\begin{pmatrix}
   0.7701334  & 0.2566214 \\
   0.2566214 & 1.0798685 
\end{pmatrix}$
  & 0.55 \\
