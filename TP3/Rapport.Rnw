...
Soit $E=\frac{1}{m}\sum 1_{z_i\ne z_i}$, $T_i=1_{z_i\ne z_i}$ suit une loi de Bernouilli de paramètre $\varepsilon$ car %$\mathbb{P}(T_1=1)=\mathbb{E}(E)=\mathbb{E}(\frac{1}{m}\overset{m}{\underset{i}{\sum}}T_i)=\frac{1}{m}\mathbb{E}(\overset{m}{\underset{i}{\sum}}{T_i})=m\overset{m}{\underset{i}{\sum}} \mathbb{E}(T_1)=\frac{m \varepsilon}{m}=\varepsilon$.

Nous répétons N fois une loi de Bernouilli, nous obtenons donc une loi binomiale $E_j {}\widetilde{}\mathcal{B}(N, \varepsilon)$ donc %$\mathbb{E}(E_j)=N\varepsilon$
