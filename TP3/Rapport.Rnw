...
Nous répétons N fois une loi de Bernouilli, nous obtenons donc une loi binomiale $E_j \sim  \mathcal{B}(N, \varepsilon)$ donc $E(E_j)=N\varepsilon$
et  $Var(E_j)=N\varepsilon(1-\varepsilon)$.

Notons $\bar{E}=\overset{N}{\underset{j}{\sum}}\frac{E_j}{N}$, 
$E(\bar{E})=E(\overset{N}{\underset{j}{\sum}}\frac{E_j}{N})=\frac{1}{N}E(\overset{N}{\underset{j}{\sum}}E_j)=\frac{1}{N}\overset{N}{\underset{j}{\sum}}E(E_j)=\frac{N\varepsilon}{N}= \varepsilon$ car $E_j$ sont indépendantes entre elles.

$Var(\bar{E})=Var(\overset{N}{\underset{j}{\sum}}\frac{E_j}{N})=\frac{1}{N^2}Var(\overset{N}{\underset{j}{\sum}}E_j)=\frac{1}{N^2}\overset{N}{\underset{j}{\sum}}Var(E_j)=\frac{N\varepsilon(1-\varepsilon)}{N^2}=\frac{\varepsilon(1-\varepsilon)}{N}$.

Nous observons bien que plus N est grand, plus la variance diminue.
Nous pouvons donc appliquer le TLC car les conditions sont vérifiées n>30. \\ 
$E_j \sim \mathcal{N}(E(E_j), Var(E_j))$, $E_j \sim \mathcal{N}(N\varepsilon, N\varepsilon(1-\varepsilon))$. \\ De même, $\bar{E} \sim \mathcal{N}(\varepsilon, \frac{\varepsilon(1-\varepsilon)}{N})$ puis $(\bar{E}-\varepsilon) \sim \mathcal(0, \frac{\varepsilon(1-\varepsilon)}{N})$ et pour finir \\
$(\bar{E}-\varepsilon)\sqrt{\frac{N}{\varepsilon(1-\varepsilon)}} \sim \mathcal{N}(0,1)$ \\
$P(u_{\alpha/2}<(\bar{E}-\varepsilon)\sqrt{\frac{N}{\varepsilon(1-\varepsilon)}} < u_{1-\alpha/2})=1 - \alpha$
$P(u_{\alpha/2}\sqrt{\frac{\varepsilon(1-\varepsilon)}{N}}< \bar{E}-\varepsilon < u_{1-\alpha/2}\sqrt{\frac{\varepsilon(1-\varepsilon)}{N}})=1 - \alpha$
$P(u_{\alpha/2}\sqrt{\frac{\varepsilon(1-\varepsilon)}{N}}+\varepsilon< \bar{E} < u_{1-\alpha/2}\sqrt{\frac{\varepsilon(1-\varepsilon)}{N}}+\varepsilon)=1 - \alpha$

Pour conclure l'intervalle de confiance est donc de la forme \\
$IC = [u_{\alpha/2}\sqrt{\frac{\varepsilon(1-\varepsilon)}{N}}+\varepsilon, u_{1-\alpha/2}\sqrt{\frac{\varepsilon(1-\varepsilon)}{N}}+\varepsilon]$
\subsection*{Sript}
%%TODO
